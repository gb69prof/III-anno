...
self.addEventListener('activate', event => event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(key => key !== VERSION && key.startsWith(String(VERSION).includes("-v") ? String(VERSION).replace(/-v.*$/i, "-") : String(VERSION))).map(key => caches.delete(key)))).then(() => self.clients.claim())));
